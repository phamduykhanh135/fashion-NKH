...
class _Item_ConfirmState extends State<Item_Confirm> {
  void updateConfirmState() async {
              ...
              actions: [
                Center(
                  ...
                  child: Text(
                    "OK",
                    style: TextStyle(
                        color: Colors.purpleAccent.shade400, fontSize: 15),
                  ),
                ))
              ],
            );
    ...
    try {
      ...
      for (int i = 0; i < n; i++) {
        if (widget.bill.items[i]['size'] == 'L') {
          //Size L
          // Lấy giá trị hiện tại từ Firestore
          DocumentSnapshot<Map<String, dynamic>> docSnapshot = await Products
              .firestore
              .collection('products')
          ...
          if (int.tryParse(currentSizeL) != null) {
            int iSizeL = int.parse(currentSizeL) -
                int.parse(widget.bill.items[i]['quality']);
            ...
            print(
                'Giá trị hiện tại của sizeL không phải là một số nguyên hợp lệ.');
          }
        } else if (widget.bill.items[i]['size'] == 'M') {
          //Size M
          // Lấy giá trị hiện tại từ Firestore
          DocumentSnapshot<Map<String, dynamic>> docSnapshot = await Products
              .firestore
          ...
          if (int.tryParse(currentSizeM) != null) {
            int iSizeM = int.parse(currentSizeM) -
                int.parse(widget.bill.items[i]['quality']);
        ...
        } else if (widget.bill.items[i]['size'] == 'S') {
          //Size S
          // Lấy giá trị hiện tại từ Firestore
          DocumentSnapshot<Map<String, dynamic>> docSnapshot = await Products
              .firestore
          ...
          if (int.tryParse(currentSizeS) != null) {
            int iSizeS = int.parse(currentSizeS) -
                int.parse(widget.bill.items[i]['quality']);
        ...
        } else if (widget.bill.items[i]['size'] == 'XL') {
          //Size XL
          // Lấy giá trị hiện tại từ Firestore
          DocumentSnapshot<Map<String, dynamic>> docSnapshot = await Products
              .firestore
          ...
          if (int.tryParse(currentSizeXL) != null) {
            int iSizeXL = int.parse(currentSizeXL) -
                int.parse(widget.bill.items[i]['quality']);
            ...
            Row(
              ...
              children: [
                ...
                Container(
                    ...
                    child: Image.network("${widget.bill.items[0]['image']}",
                        fit: BoxFit.cover)),
                ...
                Container(
                  ...
                  child: Column(
                      ...
                      children: [
                        ...
                        //Đơn giá
                        Text(
                          "${widget.bill.items[0]['price']} đ",
                          style: const TextStyle(fontSize: 15, color: Colors.red),
                        ),
                        //Số lượng
                        Text(
                          "Số lượng: ${widget.bill.items[0]['quality']}",
                          style: const TextStyle(fontSize: 15),
                        ),
                        //Size
                        Text(
                          "Size: ${widget.bill.items[0]['size']}",
                          style: const TextStyle(fontSize: 15),
                        ),
                      ]),
                )
